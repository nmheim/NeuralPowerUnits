...
npuloss(w1,w2) = mean(abs2, vec(npu(X, [w1 w2])) .- X[1,:] .* X[2,:])
nmuloss(w1,w2) = mean(abs2, vec(nmu(X, [w1 w2])) .- X[1,:] .* X[2,:])
gatednpuloss(w1,w2,g) = mean(abs2, vec(gatednpu(X, [w1 w2], [g,g])) .- X[1,:] .* X[2,:])
