...
expres, _ = produce_or_load(datadir("exp_log_sin"),
                         Dict(:lowlim=>-5, :uplim=>5, :dim=>10, :niter=>100000, :lr=>1e-3, :βl1=>0.1),
                         c -> run(c, exp),
                         prefix="npux_exp",
...
sinres, _ = produce_or_load(datadir("exp_log_sin"),
                         Dict(:lowlim=>-5, :uplim=>5, :dim=>10, :niter=>50000, :lr=>1e-3, :βl1=>0.1),
                         c -> run(c, sin),
                         prefix="npux_sin",
                         force=true, digits=8)
...
plot!(p2, xt, vec(logmodel(reshape(xt,1,:))), lw=2, label="NPUX", size=(300,200))
vline!(p2, [lowlim, uplim], lw=2, c=:gray, label="Train range")
savefig(p2, plotsdir("exp_log_sin", "npux_log.tikz"))
display(p2)
...
plot!(p3, xt, vec(sinmodel(reshape(xt,1,:))), lw=2, label="NPUX", size=(300,200))
...
savefig(p3, plotsdir("exp_log_sin", "npux_sin.tikz"))
display(p3)
